...
def call_openai(question):
    ...
    completion = oa_client.chat.completions.create(
        ...
        messages=[
            {
                "role": "user",
                "content": f"Respond like Master Chief from Halo to the following question: {question}",
            },
        ]
    )
...
async def on_message(message):
    ...
    if message.content.startswith('$halo'):
        print(f"Message: {message.content}")
        message_content = message.content.split("$halo")[1]
        print(f"Question: {message.content}")
        response = call_openai(message_content)
        print(f"Assistant: {response}")
        print("---")
...
client.run(os.getenv('DISCORD_MASTER-CHIEF_TOKEN'))
